    ...
    <section class="content overflow-x-scroll rounded-lg overflow-y-scroll h-full mx-auto py-5 px-5 min-h-screen">
        ...
        <div class="mb-3">
            ...
            <div>
                ...
                {{-- livewire table data --}}
                <livewire:youtube.youtube-table/>

                {{-- modal form --}}
                @include('livewire.youtube._youtube-form')

                {{-- confirm delete --}}
                @include('livewire.youtube._youtube-confirm')
...
@push("scripts")
    <script>
        ...
        @this.set('youtube.youtube_id', id);
        });
        Livewire.on("refreshDt", (showNoty = false) => {
            Livewire.components.getComponentsByName('youtube.youtube-table')[0].$wire.$refresh();
            if (showNoty) {
