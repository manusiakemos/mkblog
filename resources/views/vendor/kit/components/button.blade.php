...
@if ($variant == 'rounded')
    <button
        {!! $attributes->merge([
            'type' => 'button',
            'class' => 'm-1 py-2 px-4 rounded hover:shadow rounded transform hover:scale-105 transition-all flex flex-nowrap justify-center items-center dark:bg-primary-800',
        ]) !!}>
        {{ $slot }}
    </button>
...
@elseif($variant == 'link')
    <a {{ $attributes->merge(['class' => 'm-1 py-2 px-4 transform hover:scale-105 transition-all dark:bg-primary-800']) }}>
        {{ $slot }}
    </a>
@endif
